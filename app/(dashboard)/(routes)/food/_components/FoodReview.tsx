...
export default function FoodReview({ foodPhotoId, userId, close }: FoodReviewProps) {
    ...
    // Fetch reviews once when modal opens
    useEffect(() => {
        if (!foodPhotoId || fetchedReviews.current) return;
    ...
    // Submit review and immediately update UI
    const submitReview = useCallback(async () => {
        ...
        try {
            ...
            setReviews((prev) => [{ ...addedReview, username }, ...prev]);
    ...
    }, [newReview, rating, userId, foodPhotoId]);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4">
            <div className="bg-white dark:bg-black rounded-lg shadow-lg w-full max-w-lg p-6 relative">
                <button onClick={close} className="absolute top-2 right-2 text-gray-600 dark:text-red-800 dark:hover:text-red-500">
                    <FaTimes size={18} />
                </button>
