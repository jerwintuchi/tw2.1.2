...
export default function FoodList({ userId, photos, deletePhoto, updatePhotoName }: FoodListProps) {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {photos.map((photo) => {
                const isOwner = userId === photo.user_id;

                return (
                    <Card key={photo.id} className={`relative ${isOwner ? "bg-slate-800" : "bg-white dark:bg-black"}`}>
                        <CardContent className="flex flex-col items-center p-4 ">
                            ...
                            {editingPhotoId === photo.id ? (
                                <div className="flex items-center gap-2 mt-2 ">
                                    ...
                                    <Button
                                        onClick={() => {
                                            updatePhotoName(photo.id, newPhotoName);
                                ...
                                <h3 className={`font-semibold mt-2 ${isOwner ? "text-slate-300" : "dark:text-white"}`}>
                                    {photo.photo_name}
                                    {isOwner && (
                                        <button
